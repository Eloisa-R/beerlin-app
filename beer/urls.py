...
urlpatterns = [
    ...
    url(r'^(?P<beer_name>[\w0-9 \-/()""&,]+)/(?P<style_id>[0-9]+)/detail$',
        views.beer_detail, name='beer_detail'),
    url(r'^(?P<beer_name>[\w0-9 \-/()""&,]+)/detail$',
        views.beer_detail, name='beer_detail'),
    url(r'^(?P<beer_name>[\w0-9 \-/()""&,]+)/similar$',
        views.similar_beers, name='similar_beers'),
    url(r'^search/styles/$', views.styles, name='styles'),
    url(r'^styles/(?P<style_name>[\w0-9 \-/()""&,]+)/(?P<style_id>[0-9]+)/$',
        views.style_detail, name='style_detail'),
    url(r'^(?P<beer_name>[\w0-9 \-/()""&,]+)/not_found',
        views.beer_not_found, name='beer_not_found'),
    url(r'^(?P<beer_name>[\w0-9 \-/()""&,]+)/different_styles',
