...
class Beer_lookup:
    ...
    mono_beer_json = collections.OrderedDict([('Name', ['not found', 'name']),
                                              ('Style_name',
                                              ['not found', 'style', 'name']),
                                              ('Style_id', [0, 'styleId']),
                                              ('Min_Alcohol', [
                                               0.0, 'style', 'abvMin']),
                                              ('Max_Alcohol', [0.0,
                                              ...
                                              ['not found', 'style',
                                               'description']),
                                              ('Name_display',
                                              ['not found', 'nameDisplay'])])

    breweries_json = collections.OrderedDict([('Name', ['not found', 'brewery',
                                               'name']),
                                               ('Location_type', ['not found',
                                                'locationTypeDisplay']),
                                               ('Brand_classification', ['not found',
                                                'brewery', 'brandClassification']),
                                               ('Website', ['not found', 'brewery',
                                                'website']),
                                               ('Is_organic', ['N', 'brewery',
                                                'isOrganic']),
    ...
    def beer(self, name):
        setattr(Beer_lookup, 'base_url', Beer_lookup.url + Beer_lookup.API_key)
        beer_response = requests.get(Beer_lookup.base_url,
                                     params={'name': name}).json()
        try:
            setattr(Beer_lookup, 'res_num', beer_response['totalResults'])
        except KeyError:
            try:
                response = Beer_lookup.pick_right_beer(self, name)
            except KeyError:
                return ['beer_not_found', name]
            else:
                return response
        else:
            setattr(Beer_lookup, 'raw_response', beer_response)
            if Beer_lookup.res_num == 1:
                Beer_lookup.set_mono_beer_json_data(self)
                Beer_lookup.lookup(self)
                Beer_lookup.search_similar_beers(self)
                setattr(Beer_lookup, 'raw_response', {})
                beer_name = Beer_lookup.beer_json_response.get('Name')
                setattr(Beer_lookup, 'beer_json_response', {})
                return ['beer_detail', beer_name]
            else:
                response = Beer_lookup.show_styles(self, name)
                setattr(Beer_lookup, 'raw_response', {})
                return response
    ...
    a record in the Beers table with the style_id foreing key'''

    def lookup(self):
        for key, value in Beer_lookup.mono_beer_json.items():
            Beer_lookup.beer_json_response[key] = Beer_lookup.json_helper(
                self, Beer_lookup.beer_json_data, value)
        new_entry_obj, created = Styles.objects.get_or_create(defaults={
            'style_id': Beer_lookup.beer_json_response.get('Style_id'),
            'style_name': Beer_lookup.beer_json_response.get('Style_name'),
            'description': Beer_lookup.beer_json_response.get('Description')},
            style_id__exact=Beer_lookup.beer_json_response.get(
            'Style_id'),
            style_name__iexact=Beer_lookup.beer_json_response.get(
                'Style_name'),
            description__iexact=Beer_lookup.beer_json_response.get(
                'Description'))
        new_entry_obj.beers_set.get_or_create(defaults={
            'name': Beer_lookup.beer_json_response.get('Name'),
            'display_name': Beer_lookup.beer_json_response.get('Name_display'),
            'min_alcohol': Beer_lookup.beer_json_response.get('Min_Alcohol'),
            'max_alcohol': Beer_lookup.beer_json_response.get('Max_Alcohol'),
            'min_bitter': Beer_lookup.beer_json_response.get('Min_Bitterness'),
            'max_bitter': Beer_lookup.beer_json_response.get(
                'Max_Bitterness')},
            name__iexact=Beer_lookup.beer_json_response.get('Name'))
        setattr(Beer_lookup, 'beer_json_data', {})
    ...
    def pick_right_beer(self, name):
        search_response = requests.get(
            'http://api.brewerydb.com/v2/search?q=' + str(
                name) + '&type=beer&key=' + Beer_lookup.API_key).json()
        for item in search_response['data']:
            Beer_lookup.similar_beers_set.add(
                Beer_lookup.json_helper(
                    self, item, Beer_lookup.mono_beer_json.get(
                        'Name_display')))
        for item in Beer_lookup.similar_beers_set:
            Similar_beers.objects.get_or_create(defaults={
                'common_name': name,
                'beer_name': item},
                common_name__iexact=name,
                beer_name__iexact=item)
        setattr(Beer_lookup, 'similar_beers_set', set())
    ...
    def set_mono_beer_json_data(self):
        setattr(Beer_lookup, 'beer_json_data',
                Beer_lookup.raw_response['data'][0])
    ...
    def json_helper(self, temp_json_data, json_ind_list):
        setattr(Beer_lookup, 'temp_json_data', temp_json_data)
        for item in json_ind_list[1:]:
            try:
                result = Beer_lookup.temp_json_data[item]
            except KeyError:
                result = json_ind_list[0]
                break
            else:
                setattr(Beer_lookup, 'temp_json_data', result)
        return result
    ...
    def search_similar_beers(self):
        similar_beers_response = requests.get(
            Beer_lookup.base_url,
            params={'styleId': Beer_lookup.beer_json_response.get(
                'Style_id')}).json()
        for item in similar_beers_response['data']:
            Beer_lookup.similar_set.add(Beer_lookup.json_helper(
                self, item, Beer_lookup.mono_beer_json.get(
                    'Name_display')))
        recent_entry = Styles.objects.get(
            style_id=Beer_lookup.beer_json_response.get('Style_id'))
        for item in Beer_lookup.similar_set:
            recent_entry.beers_per_style_set.get_or_create(
                defaults={'beer_name': item}, beer_name__iexact=item)
        setattr(Beer_lookup, 'similar_set', set())
    ...
    def show_styles(self, beer_name):
        Beer_lookup.set_beer_json_data(self, Beer_lookup.raw_response['data'])
        for item in Beer_lookup.beer_json_data:
            style_id = Beer_lookup.json_helper(
                self, item, Beer_lookup.mono_beer_json.get('Style_id'))
            style_name = Beer_lookup.json_helper(
                self, item, Beer_lookup.mono_beer_json.get('Style_name'))
            if style_id != 'not found' and style_name != 'not found':
                Beer_lookup.styles_dict[style_id] = style_name
        for key, value in Beer_lookup.styles_dict.items():
        ...
        setattr(Beer_lookup, 'beer_json_data', {})
        setattr(Beer_lookup, 'styles_dict', {})
    ...
    def select_by_style(self, beer_name, style_id):
        setattr(Beer_lookup, 'base_url', Beer_lookup.url + Beer_lookup.API_key)
        styles_response = requests.get(Beer_lookup.base_url,
                                       params={'name': beer_name,
                                               'styleId': style_id}).json()
        setattr(Beer_lookup, 'raw_response', styles_response)
        Beer_lookup.set_mono_beer_json_data(self)
        Beer_lookup.lookup(self)
        Beer_lookup.search_similar_beers(self)
        beer_name = Beer_lookup.beer_json_response.get(
            'Name')
        setattr(Beer_lookup, 'raw_response', {})
        setattr(Beer_lookup, 'beer_json_response', {})
    ...
    def styles(self):
        styles_response = requests.get(
            ...
            Beer_lookup.API_key).json()
        for item in styles_response['data']:
            Beer_lookup.styles_dict[Beer_lookup.json_helper(
                self, item, Beer_lookup.style_id)] = [Beer_lookup.json_helper(
                    self, item, Beer_lookup.mono_beer_json.get('Name')),
                Beer_lookup.json_helper(
                    self, item, Beer_lookup.mono_beer_json.get(
                        'Description')[1:])]
        for key, value in Beer_lookup.styles_dict.items():
    ...
    def style_detail(self, style_id, style_name):
        setattr(Beer_lookup, 'base_url', Beer_lookup.url + Beer_lookup.API_key)
        beers_style_response = requests.get(Beer_lookup.base_url,
                                            params={
                                                'styleId': style_id}).json()
        for item in beers_style_response['data']:
            Beer_lookup.style_detail_dict[Beer_lookup.json_helper(
                self, item, Beer_lookup.mono_beer_json.get(
                    'Name_display'))] = Beer_lookup.json_helper(
                self, item, Beer_lookup.mono_beer_json.get('Description'))

        for key, value in Beer_lookup.style_detail_dict.items():
            styobj, created = Styles.objects.get_or_create(
        ...
        setattr(Beer_lookup, 'style_detail_dict', {})

    def get_breweries(self):
        response = requests.get('http://api.brewerydb.com/v2/locations/?key='
                                + str(Beer_lookup.API_key), params={
                                'region': 'Berlin'}).json()
        for item in response['data']:
            for key, value in Beer_lookup.breweries_json.items():
                Beer_lookup.breweries_dict[key] = Beer_lookup.json_helper(
                    self, item, value)
            Breweries.objects.get_or_create(defaults={'name':
                Beer_lookup.breweries_dict.get('Name'),
                'location_type': Beer_lookup.breweries_dict.get(
                    'Location_type'),
                'brand_classification': Beer_lookup.breweries_dict.get(
                    'Brand_classification'),
                'website': Beer_lookup.breweries_dict.get('Website'),
                'is_organic': Beer_lookup.breweries_dict.get('Is_organic'),
                'longitude': Beer_lookup.breweries_dict.get('Longitude'),
                'latitude': Beer_lookup.breweries_dict.get('Latitude'),
                 'address': Beer_lookup.breweries_dict.get('Address')},
                name__iexact=Beer_lookup.breweries_dict.get('Name'))
            setattr(Beer_lookup, 'breweries_dict', {})
            setattr(Beer_lookup, 'beer_json_data', {})
            setattr(Beer_lookup, 'temp_json_data', {})
