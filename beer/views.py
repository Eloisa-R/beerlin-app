...
def beer_detail(request, beer_name, style_id=None):
    if style_id is not None:
        ...
        [similar_list.remove(item)
         for item in similar_list
         if beer.name.lower() == item.lower()]
    else:
...
def similar_beers(request, beer_name):
    if request.method == 'POST':
        ...
        context = {'title': 'Pick the right beer',
                   'text': 'There are several beers with this name, \
                   please choose one.',
...
def styles(request):
    ...
    for item in style_search:
        global_dict[item.style_name[0]].append(
...
def styles_in_beer(request, beer_name):
    if request.method == 'POST':
        ...
        context = {'title': 'Pick the right style',
                   'text': 'There are several beers with \
        ...
        if select_form.is_valid():
            ...
            context = {'title': 'Pick the right style',
                       'text': 'There are several beers with this name \
                        that have different styles, please pick one.',
                       'select_form': select_form,
                       'rangeb': rangeb,
                       'num_col': num_col,
                       'invalid': 'Please select an option'
...
def beer_not_found(request, beer_name):
    context = {'text': 'Sorry, I couldn\'t find the beer ' +
               beer_name + ', please try again',
               'subtitle': 'Not found'}
...
def about(request):
    ...
    text = 'The Beerlin project revolves around two great things ' \
           ...
           'characteristics of an specific beer' \
           'by entering its name—especially ' \
           ...
           '(website, type of beer produced, etc.) and ' \
...
def breweries(request):
    ...
    text = 'This is a list of the breweries in the Berlin region.' \
